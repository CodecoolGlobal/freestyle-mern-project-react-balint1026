...
async function start() {
  try {
    await mongoose.connect(
      `mongodb+srv://${process.env.DB_USERNAME}:${process.env.DB_PASSWORD}@cluster0.8u3exxo.mongodb.net/project`
    );
...
app.get("/api/users", (req, res) => {
  userModel
    .find(req.query.username ? { username: req.query.username } : {})
    .sort({ username: 1 })
...
app.post("/api/events", async (req, res) => {
  try {
    const { host, name, description, attendees, location, date, price } =
      req.body;
